...
| 집계함수 | ❌ 불가 | ✅ 가능 |
        """)
    with c2:
        st.markdown('''<div class="wrn"><b>GROUP BY 규칙</b><br>SELECT의 비집계 컬럼은 반드시 GROUP BY에 포함해야 합니다.<br><br>✅ <code>SELECT basis_ym, pnsn_gv_meth_dvcd, COUNT(*)</code><br><code>GROUP BY basis_ym, pnsn_gv_meth_dvcd</code><br><br>❌ <code>GROUP BY basis_ym</code> ← pnsn_gv_meth_dvcd 누락!</div>''', unsafe_allow_html=True)
